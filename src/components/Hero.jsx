...
export const Hero = () => {
  ...
  return (
    <div className="h-[350px] sm:h-[520px] w-full ">
      <video className="w-full h-[350px] sm:h-[520px] brightness-[60%] object-cover " autoPlay muted loop poster={`${random.thumbnailUrl}`} src={`${random.videoUrl}`}></video>
      <div className="desc-hero absolute lg:top-[20%] sm:top-[12%] top-[10%] left-[5%] max-w-xs sm:max-w-xl ">
        <p className="text-white text-smtitle sm:text-title font-bold">{random.title}</p>
        <p className="text-primary text-smtprimary mt-2 sm:text-tprimary">{random.description}</p>
        <a href={hrefValue}>
